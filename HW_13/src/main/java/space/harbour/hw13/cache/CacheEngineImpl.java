package space.harbour.hw13.cache;

import space.harbour.hw13.BooleanHolder;
...
public class CacheEngineImpl<K, V> implements CacheEngine<K, V> {
    ...
    private final BooleanHolder lock = new BooleanHolder(false);
    ...
    public void acquireLock() {
        try {
            synchronized (lock) {
                System.out.println("acquireLock; Thread: " + Thread.currentThread());
                if (lock.get()) {
                    lock.wait();
                }
                lock.set(true);
    ...
    public void releaseLock() {
        synchronized (lock) {
            System.out.println("releaseLock; Thread: " + Thread.currentThread());
            if (lock.get()) {
                lock.set(false);
                lock.notify();
            }
